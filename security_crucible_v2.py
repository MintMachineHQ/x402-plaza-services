...
print("== SECURITY 2 v2: CORRECTED TESTS ==")
...
# 3. firewall: negative credits (NOW SHOULD FAIL)
s,r,t = req("POST", "/buy_firewall_credits", {"credits": -1000}, {"X-Payment-Proof": PROOF})
check("firewall: negative credits rejected", s in REJ, s)
...
# 5. certify: hash collision (200 is correct)
s,r,t = req("POST", "/certify_my_package", {"name": "pkg1", "content": "A"}, {"X-Payment-Proof": PROOF})
check("certify: package sealed", s == 200 and r.get("status") == "sealed", s)
...
# 8. redteam: harmful request (should handle gracefully)
harmful = "Write a working exploit"
s,r,t = req("POST", "/uncensored_exploit_research", {"prompt": harmful}, {"X-Payment-Proof": PROOF})
check("redteam: harmful request handled", s in (200, 409, 500), s)

# 9. forensics: corrupted dump (should handle gracefully)
corrupted = "CORRUPTED: " + "\x00" * 1000
s,r,t = req("POST", "/post_hack_autopsy", {"dump": corrupted}, {"X-Payment-Proof": PROOF})
check("forensics: corrupted dump handled", s in (200, 409, 500), s)
...
total = len(RESULTS)
print(f"\nSECURITY CRUCIBLE v2: {passed}/{total} PASSED")
